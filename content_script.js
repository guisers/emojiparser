...
function watchAll(e) {
	var inputString = e.target.textContent;
	var cursor = window.getSelection().focusOffset;
	if (!inputString) {
		inputString = e.target.value;
		cursor = e.target.selectionStart;
	}	
	var input = inputString.slice(0, cursor);
	if (regex.test(input)) {
		var lastCharacter = input.slice(input.length-1);
		if (lastCharacter == ":") {
			var match = input.match(regex)[0];
			var start = match.indexOf(":");
			var emoji = match.slice(start+1, match.length-1);
			var toTruncate = input.length - (emoji.length + 2);
			var unicode = emojiMap[emoji];
			if (unicode) {
				if (e.target.value) {
					e.target.value = input.slice(0, toTruncate) + unicode + inputString.slice(cursor);
					e.target.selectionStart = cursor+1;
				} else {
					window.getSelection().focusNode.data = input.slice(0, toTruncate) + unicode + inputString.slice(cursor);
				}
			}
		}
	}
...
//   return "\u"+lead.toString(16)+"\u"+trail.toString(16);
// }
